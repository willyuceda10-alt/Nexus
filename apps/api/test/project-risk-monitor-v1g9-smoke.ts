...
async function main() {
  ...
  try {
    ...
    assert(
      firstRun.tenantsScanned >= 1,
      `Expected automatic G9 tenant discovery, got ${firstRun.tenantsScanned}`,
    );

    assert(
      firstRun.projectsScanned === 1,
      `Expected one G9 project, got ${firstRun.projectsScanned}`,
    );

    assert(
      firstRun.alertsQueued === 1,
      `Expected one queued G9 alert, got ${firstRun.alertsQueued}`,
    );

    assert(
      firstRun.alertsDeduplicated === 0,
      'First G9 run must not deduplicate',
    );
    ...
    assert(
      notificationEvent,
      'G9 notification Outbox event missing',
    );

    const payload =
      notificationEvent.payload as {
        priority?: string;
        targetUserId?: string;
    ...
    assert(
      ...
      `Expected CRITICAL G9 notification, got ${payload.priority}`,
    );

    assert(
      payload.targetUserId === userId,
      'G9 must notify project owner',
    );

    assert(
      payload.scopeProjectId === projectId,
      'G9 project scope missing',
    );
    ...
    assert(
      ...
      'G9 notification was not projected',
    );

    assert(
      projection.inboxItemId,
      'G9 Inbox item id missing',
    );
    ...
    assert(
      inbox.length === 1,
      `Expected one G9 Inbox item, got ${inbox.length}`,
    );

    assert(
      inbox[0]?.priority ===
        'CRITICAL',
      'G9 Inbox priority must be CRITICAL',
    );

    assert(
      inbox[0]?.requires_action === true,
      'G9 Inbox must require action',
    );
    ...
    assert(
      secondRun.alertsQueued === 0,
      'Second G9 run must not queue duplicate alert',
    );

    assert(
      secondRun.alertsDeduplicated === 1,
      `Expected one deduplicated G9 alert, got ${secondRun.alertsDeduplicated}`,
    );
    ...
    assert(
      Number(
        inboxCount[0]?.count ?? 0,
      ) === 1,
      'G9 redelivery created duplicate Inbox item',
    );
    ...
    console.log(JSON.stringify({
      ...
      notificationQueued: firstRun.alertsQueued === 1,
      duplicateSuppressed: secondRun.alertsDeduplicated === 1,
      inboxProjected: true,
      automaticRecipient: 'PROJECT_OWNER',
      tenantsScanned: firstRun.tenantsScanned,
      projectsScanned: firstRun.projectsScanned,
