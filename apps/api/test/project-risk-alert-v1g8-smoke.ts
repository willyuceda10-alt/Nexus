...
async function main() {
  ...
  try {
    ...
    const firstAlertResponse = await app.inject({
      method: 'POST',
      url: `/api/v1/projects/${projectId}/risk-alert-v1g8/evaluate`,
      payload: {
        targetUserId: userId,
      },
    });
    ...
    assert(
      firstAlertResponse.statusCode === 202,
      `G8 first alert evaluation failed: ${firstAlertResponse.statusCode} ${firstAlertResponse.body}`,
    );

    const firstAlert = firstAlertResponse.json();

    assert(firstAlert.version === 'v1g8', 'Unexpected G8 version');
    assert(firstAlert.risk.riskLevel === 'CRITICAL', `Expected G8 CRITICAL risk, got ${firstAlert.risk.riskLevel}`);
    assert(firstAlert.alert.shouldNotify === true, 'G8 critical risk must notify');
    assert(firstAlert.alert.priority === 'CRITICAL', `Expected CRITICAL priority, got ${firstAlert.alert.priority}`);
    assert(firstAlert.alert.notificationQueued === true, 'First G8 notification must be queued');
    assert(firstAlert.alert.deduplicated === false, 'First G8 notification must not be deduplicated');

    assert(
      JSON.stringify(firstAlert.risk) === JSON.stringify(risk),
      'G8 recalculated risk must match canonical G7 risk',
    );

    const notificationEvent = await withTenant(tenantId, async (tx) => {
      return tx.domainEvent.findFirst({
        where: {
          id: firstAlert.alert.notificationEventId,
          tenantId,
    ...
    assert(notificationEvent, 'G8 notification outbox event missing');

    const projection = await projectPlatformEventV1({
      schemaVersion: 1,
      eventId: notificationEvent.id,
      tenantId: notificationEvent.tenantId,
      aggregateId: notificationEvent.aggregateId,
      eventType: notificationEvent.eventType,
      payload: notificationEvent.payload,
      occurredAt: notificationEvent.createdAt.toISOString(),
    });

    assert(projection.handled === true, 'G8 notification event was not projected');
    assert(projection.inboxItemId, 'G8 Inbox item id missing');

    const inboxAfterFirstProjection = await withTenant(tenantId, async (tx) => {
      return tx.$queryRaw<Array<{
        ...
        SELECT id, priority::text, requires_action, unread, source_id
        FROM inbox_items_v1
        WHERE tenant_id = ${tenantId}::uuid
          AND user_id = ${userId}::uuid
          AND project_object_id = ${projectId}::uuid
          AND source_type = 'AUTOMATION_NOTIFICATION'
          AND source_id = ${notificationEvent.id}::uuid
      `);
    });

    assert(inboxAfterFirstProjection.length === 1, `Expected one G8 Inbox item, got ${inboxAfterFirstProjection.length}`);
    assert(inboxAfterFirstProjection[0]?.priority === 'CRITICAL', 'G8 Inbox priority must be CRITICAL');
    assert(inboxAfterFirstProjection[0]?.requires_action === true, 'G8 Inbox must require action');
    assert(inboxAfterFirstProjection[0]?.unread === true, 'G8 Inbox must start unread');

    const secondAlertResponse = await app.inject({
      method: 'POST',
      url: `/api/v1/projects/${projectId}/risk-alert-v1g8/evaluate`,
      payload: {
        targetUserId: userId,
    ...
    assert(
      secondAlertResponse.statusCode === 200,
      `G8 duplicate evaluation failed: ${secondAlertResponse.statusCode} ${secondAlertResponse.body}`,
    );

    const secondAlert = secondAlertResponse.json();

    assert(secondAlert.alert.notificationQueued === false, 'Duplicate G8 alert must not queue another notification');
    assert(secondAlert.alert.deduplicated === true, 'Duplicate G8 alert must report deduplicated');
    assert(
      secondAlert.alert.notificationEventId === firstAlert.alert.notificationEventId,
      'Duplicate G8 evaluation must reuse the same outbox event',
    );
    ...
    const inboxAfterRedelivery = await withTenant(tenantId, async (tx) => {
    ...
    assert(
      Number(inboxAfterRedelivery[0]?.count ?? 0) === 1,
      'G8 projector redelivery created a duplicate Inbox item',
    );

    console.log(JSON.stringify({
      projectRiskAlertV1g8: 'PASS',
      projectRiskForecastV1g7: 'PASS',
      alertPriority: firstAlert.alert.priority,
      notificationQueued: firstAlert.alert.notificationQueued,
      duplicateSuppressed: secondAlert.alert.deduplicated,
      inboxProjected: true,
    ...
    await withTenant(tenantId, async (tx) => {
      await tx.$executeRaw(Prisma.sql`
        ...
        WHERE tenant_id = ${tenantId}::uuid
          AND (
            aggregate_id = ${projectId}::uuid
            OR payload->>'projectId' = ${projectId}
            OR payload->>'scopeProjectId' = ${projectId}
          )
      `);

      await tx.$executeRaw(Prisma.sql`
        DELETE FROM notification_deliveries_v1
        WHERE tenant_id = ${tenantId}::uuid
      ...
      await tx.auditLog.deleteMany({
        where: {
          ...
          action: 'PROJECT_RISK_ALERT_EVALUATED',
        },
      });
