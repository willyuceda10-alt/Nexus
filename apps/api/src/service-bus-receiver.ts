...
export interface LockedServiceBusMessageV1 {
...
export class AzureServiceBusAutomationReceiver {
  ...
  constructor() {
    if (!config.SERVICE_BUS_NAMESPACE) throw new Error('SERVICE_BUS_NAMESPACE is required for the automation receiver.');
    ...
    this.receiveUrl = `https://${this.namespaceHost}/${encodeURIComponent(config.SERVICE_BUS_TOPIC)}`
      + `/subscriptions/${encodeURIComponent(config.SERVICE_BUS_AUTOMATION_SUBSCRIPTION)}/messages/head`;
  }
  ...
  private safeLockUrl(raw: string): string {
    ...
    if (url.protocol !== 'https:' || url.hostname.toLowerCase() !== this.namespaceHost.toLowerCase()) {
      throw new Error('Service Bus returned a lock URL outside the configured namespace.');
    }
    return url.toString();
  }

  async receive(): Promise<LockedServiceBusMessageV1 | null> {
    const url = `${this.receiveUrl}?timeout=${config.AUTOMATION_RECEIVE_TIMEOUT_SECONDS}`;
    const response = await this.authorizedFetch(url, {
      method: 'POST',
      headers: { 'Content-Length': '0' },
      signal: AbortSignal.timeout((config.AUTOMATION_RECEIVE_TIMEOUT_SECONDS + 10) * 1000),
    });
    if (response.status === 204) return null;
    if (response.status !== 201) {
      const body = (await response.text()).slice(0, 2000);
      throw new Error(`Service Bus receive failed (${response.status} ${response.statusText}): ${body}`);
    }

    const location = response.headers.get('location');
    if (!location) throw new Error('Service Bus peek-lock response did not include Location.');
    ...
    let envelope: AutomationEventEnvelopeV1;
    try {
      envelope = JSON.parse(body) as AutomationEventEnvelopeV1;
    } catch {
      throw new Error('Service Bus automation message body is not valid JSON.');
    }
    return {
      envelope,
  ...
  async complete(message: LockedServiceBusMessageV1): Promise<void> {
    ...
    if (response.status !== 200 && response.status !== 404) {
      ...
      throw new Error(`Service Bus complete failed (${response.status} ${response.statusText}): ${body}`);
    }
  }

  async abandon(message: LockedServiceBusMessageV1): Promise<void> {
    const response = await this.authorizedFetch(this.safeLockUrl(message.lockUrl), {
      method: 'PUT',
      headers: { 'Content-Length': '0' },
      signal: AbortSignal.timeout(15_000),
    ...
    if (response.status !== 200 && response.status !== 404) {
      const body = (await response.text()).slice(0, 1000);
      throw new Error(`Service Bus abandon failed (${response.status} ${response.statusText}): ${body}`);
    }
  }
