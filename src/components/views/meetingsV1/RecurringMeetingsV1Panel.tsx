...
function calendarDayOfMonth(value: string): number { return new Date(value).getDate(); }
function verifiedMonthlyDay(value: string): number { return Math.min(calendarDayOfMonth(value), 28); }
...
export const RecurringMeetingsV1Panel: React.FC<{ projectId?: string; onChanged?: () => void }> = ({ projectId, onChanged }) => {
  ...
  const [form, setForm] = useState({
    ...
    dayOfMonth: verifiedMonthlyDay(initialDates.start), rangeType: 'NUMBERED' as RecurrenceRangeTypeV1, numberOfOccurrences: 4, endDate: '',
  ...
  const updateStart = (value: string) => {
    setForm((current) => ({
      ...
      dayOfMonth: current.patternType === 'ABSOLUTE_MONTHLY' ? verifiedMonthlyDay(value) : current.dayOfMonth,
    }));
  };
  const updatePattern = (patternType: RecurrencePatternTypeV1) => {
    setForm((current) => ({
      ...
      dayOfMonth: patternType === 'ABSOLUTE_MONTHLY' ? verifiedMonthlyDay(current.startAt) : current.dayOfMonth,
    }));
  };
  ...
  const createSeries = async () => {
    ...
    if (form.patternType === 'ABSOLUTE_MONTHLY' && calendarDayOfMonth(form.startAt) > 28) { setError('Recurring Meetings V1 limita el patrón mensual a los días 1–28 hasta validar los días de borde contra Outlook/Graph.'); return; }
    ...
    <section className="command-panel overflow-hidden">
      <div className="flex flex-col gap-4 px-5 py-5 lg:flex-row lg:items-center lg:justify-between">
        <div>
          ...
          <p className="mt-1 max-w-3xl text-[9px] leading-4 text-slate-500">Diaria, semanal o mensual por día 1–28. Bridata conserva el master y cada ocurrencia; Outlook/Teams recibe una serie recurrente cuando M365 está habilitado.</p>
        </div>
      ...
      {createOpen && <div className="fixed inset-0 z-50 grid place-items-center bg-slate-950/20 p-4 backdrop-blur-[2px]" onMouseDown={(event) => { if (event.currentTarget === event.target) setCreateOpen(false); }}><div className="max-h-[92vh] w-full max-w-[980px] overflow-y-auto rounded-2xl border border-slate-200 bg-white p-5 shadow-2xl"><div className="flex items-start justify-between"><div><p className="text-[9px] font-black uppercase tracking-[0.14em] text-green-700">Nueva serie recurrente</p><h3 className="mt-1 text-[19px] font-extrabold text-slate-950">Programar patrón + participantes + recursos</h3></div><button type="button" onClick={() => setCreateOpen(false)} className="grid h-8 w-8 place-items-center rounded-lg border border-slate-200 text-slate-500"><X className="h-4 w-4" /></button></div>
        ...
        <div className="space-y-3"><div className="rounded-xl border border-slate-200 p-3"><p className="text-[8px] font-black uppercase text-slate-400">Patrón</p><div className="mt-2 grid grid-cols-3 gap-1">{(['DAILY','WEEKLY','ABSOLUTE_MONTHLY'] as RecurrencePatternTypeV1[]).map((value)=><button key={value} type="button" onClick={()=>updatePattern(value)} className={`rounded-lg border px-2 py-2 text-[7px] font-black ${form.patternType===value?'border-green-300 bg-green-50 text-green-800':'border-slate-200 text-slate-500'}`}>{value==='DAILY'?'Diaria':value==='WEEKLY'?'Semanal':'Mensual 1–28'}</button>)}</div><label className="mt-3 block text-[8px] font-black uppercase text-slate-400">Intervalo<input type="number" min={1} max={99} value={form.interval} onChange={(e)=>setForm({...form,interval:Number(e.target.value)})} className="mt-1 h-9 w-full rounded-lg border border-slate-200 px-3 text-[9px]" /></label>{form.patternType==='WEEKLY'&&<div className="mt-3 flex flex-wrap gap-1">{DAYS.map((item)=><button key={item.key} type="button" onClick={()=>toggleDay(item.key)} className={`rounded-lg border px-2 py-1.5 text-[7px] font-black ${form.daysOfWeek.includes(item.key)?'border-green-300 bg-green-50 text-green-800':'border-slate-200 text-slate-500'}`}>{item.label}</button>)}</div>}{form.patternType==='ABSOLUTE_MONTHLY'&&<label className="mt-3 block text-[8px] font-black uppercase text-slate-400">Día del mes<input type="number" min={1} max={28} value={form.dayOfMonth} onChange={(e)=>setForm({...form,dayOfMonth:Math.min(28,Math.max(1,Number(e.target.value)))})} className="mt-1 h-9 w-full rounded-lg border border-slate-200 px-3 text-[9px]" /></label>}</div>
