    ...
    <div className="mx-auto w-full max-w-[1540px] space-y-6 px-6 py-6 lg:px-8">
      <section className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
        <div>
          ...
          <p className="mt-1 max-w-3xl text-[12px] leading-5 text-slate-500">Convierte reuniones en decisiones trazables y acciones reales. Bridata enlaza agenda, minuta, acuerdos y tareas sin fabricar identificadores ni duplicar el registro de ejecución.</p>
        </div>

        <div className="flex flex-wrap gap-2">
          <button onClick={() => openCreateModal('MEETING')} className="inline-flex items-center gap-2 rounded-xl border border-green-200 bg-green-50 px-3.5 py-2.5 text-[11px] font-bold text-green-800 transition hover:bg-green-100"><Plus className="h-4 w-4" /> Agendar reunión</button>
          <button onClick={() => openCreateModal('DECISION')} className="inline-flex items-center gap-2 rounded-xl bg-green-700 px-3.5 py-2.5 text-[11px] font-bold text-white shadow-sm transition hover:bg-green-800"><Plus className="h-4 w-4" /> Registrar decisión</button>
        </div>
      </section>
      ...
      <section className="command-panel p-4">
        <div className="relative max-w-[560px]">
          ...
          <input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Buscar reunión, agenda, participante, decisión o proyecto…" className="h-10 w-full rounded-xl border border-slate-200 bg-white pl-9 pr-3 text-[11px] font-medium text-slate-800 outline-none transition focus:border-green-400 focus:ring-2 focus:ring-green-100" />
        </div>
      </section>
              ...
              <article key={meeting.id} className="group px-5 py-4 transition hover:bg-slate-50/70">
                <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
                  <button onClick={() => openObjectDrawer(meeting.id)} className="min-w-0 flex-1 text-left">
                    ...
                    <div className="mt-3 flex flex-wrap items-center gap-2 text-[9px] text-slate-500"><span className="inline-flex items-center gap-1 rounded-lg bg-slate-100 px-2 py-1"><UsersRound className="h-3 w-3" /> {meeting.participants.length} participantes</span><span className="inline-flex items-center gap-1 rounded-lg bg-slate-100 px-2 py-1"><ListChecks className="h-3 w-3" /> {meeting.decisionIds.length} decisiones</span><span className="inline-flex items-center gap-1 rounded-lg bg-slate-100 px-2 py-1"><Link2 className="h-3 w-3" /> {meeting.actionIds.length} acciones</span></div>
                  </button>

                  <div className="flex shrink-0 flex-wrap gap-2">
                    <button onClick={() => setEditorObjectId(meeting.id)} className="inline-flex items-center gap-1 rounded-lg border border-slate-200 bg-white px-2.5 py-1.5 text-[9px] font-bold text-slate-600 hover:border-green-200 hover:text-green-800"><Pencil className="h-3 w-3" /> Editar</button>
                    {!TERMINAL_MEETING_STATUSES.has(meeting.status) && <button disabled={isObjectMutationPending} onClick={() => void handleCompleteMeeting(meeting)} className="rounded-lg border border-slate-200 bg-white px-2.5 py-1.5 text-[9px] font-bold text-slate-600 transition hover:border-green-200 hover:text-green-800 disabled:opacity-50">Marcar realizada</button>}
                    <button disabled={!meeting.projectId || actionMeetingId !== null || isObjectMutationPending} onClick={() => void handleCreateTaskFromMeeting(meeting)} title={!meeting.projectId ? 'La reunión debe estar asociada a un proyecto' : 'Crear una tarea real vinculada a esta reunión'} className="rounded-lg bg-green-700 px-2.5 py-1.5 text-[9px] font-bold text-white transition hover:bg-green-800 disabled:cursor-not-allowed disabled:opacity-45">{actionMeetingId === meeting.id ? 'Creando…' : 'Derivar acción'}</button>
                  </div>
                </div>
              </article>
            ))}
            {filteredMeetings.length === 0 && <div className="px-6 py-12 text-center"><CalendarDays className="mx-auto h-7 w-7 text-slate-300" /><p className="mt-3 text-[11px] font-bold text-slate-600">No hay reuniones que coincidan con el filtro.</p><p className="mt-1 text-[9px] text-slate-400">Agenda una nueva reunión o cambia la vista.</p></div>}
          </div>
        </div>
              ...
              <article key={decision.id} className="group px-5 py-4 transition hover:bg-slate-50/70">
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex shrink-0 flex-col gap-2">
                    <button onClick={() => setEditorObjectId(decision.id)} className="inline-flex items-center gap-1 rounded-lg border border-slate-200 bg-white px-2.5 py-1.5 text-[9px] font-bold text-slate-600 hover:border-green-200 hover:text-green-800"><Pencil className="h-3 w-3" /> Editar</button>
                    <button onClick={() => openObjectDrawer(decision.id)} className="grid h-8 w-8 place-items-center self-end rounded-lg text-slate-300 hover:bg-white hover:text-green-600"><ArrowUpRight className="h-4 w-4" /></button>
                  </div>
                </div>
              </article>
            ))}
