    ...
    <div className="fixed inset-0 z-50 grid place-items-center bg-slate-950/20 p-4 backdrop-blur-[2px]" onMouseDown={(event) => { if (event.currentTarget === event.target) onClose(); }}>
      <div className="w-full max-w-[520px] rounded-2xl border border-slate-200 bg-white p-5 shadow-2xl">
        <div className="flex items-start justify-between">
          ...
          <button onClick={onClose} className="grid h-8 w-8 place-items-center rounded-lg border border-slate-200 text-slate-500"><X className="h-4 w-4" /></button>
        </div>
        ...
        <button disabled={saving || !name.trim() || !startFieldKey} onClick={() => void onCreate({ name: name.trim(), viewType, config: { startFieldKey, endFieldKey: endFieldKey || null, titleFieldKey: 'title', colorFieldKey: 'status', allDay: true } })} className="mt-5 h-10 w-full rounded-xl bg-green-700 text-[10px] font-black text-white disabled:opacity-40">Crear vista</button>
      </div>
    </div>
  );
}
