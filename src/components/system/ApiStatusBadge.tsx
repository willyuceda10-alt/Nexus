...
export function ApiStatusBadge() {
  ...
  if (status === 'mock') {
    return (
      <span
        ...
        title="La interfaz todavía usa datos mock. Cambia VITE_DATA_MODE=api para conectar la API."
      >
  ...
  if (status === 'loading') {
    return (
      <span className="hidden xl:inline-flex items-center gap-1.5 rounded-full border border-indigo-100 bg-indigo-50 px-2.5 py-1 text-[10px] font-semibold text-indigo-600">
  ...
  if (status === 'error') {
    return (
      <button
        ...
        title={error || 'No se pudo conectar con la API. Haz clic para reintentar.'}
      >
  ...
  return (
    <span
      ...
      title={bootstrap ? `${bootstrap.tenant.name} · ${bootstrap.actor.email}` : 'API conectada'}
    >
